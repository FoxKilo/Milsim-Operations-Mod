...
if ( !isNull _commanderobj ) then {
	...
	if (_uid in GRLIB_whitelisted_steamids) then {
		_uidcheckcomplete = ["[COC Gaming]", player, "is whitelisted for commander"];
		}
	if (if (_uid in !GRLIB_whitelisted_steamids) then {
		_uidcheckcomplete = ["[COC Gaming]", player, "is NOT whitelisted for commander"];
		}
